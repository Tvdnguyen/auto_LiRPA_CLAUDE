...
class MaskedPerturbationLpNorm(PerturbationLpNorm):
    ...
    def init(self, x, aux=None, forward=False):
        ...
        Args:
            ...
            forward: Whether using forward mode
        ...
        if not forward:
            # Backward mode: return LinearBound with bounds only
            return LinearBound(None, None, None, None, x_L, x_U), x, aux
        else:
            # Forward mode: create identity matrices for perturbed dimensions
            batch_size = x.shape[0]

            if len(x.shape) == 4:  # Conv layer
                # For conv layers, create sparse identity for perturbed elements
                dim = x.numel() // batch_size
                x_flat = x.reshape(batch_size, -1)
                mask_flat = self.full_mask.reshape(batch_size, -1)

                # Create identity matrix
                eye = torch.eye(dim, device=x.device).unsqueeze(0).repeat(batch_size, 1, 1)

                # Mask out non-perturbed dimensions
                eye = eye * mask_flat.unsqueeze(1).float()
            ...
            elif len(x.shape) == 2:  # FC layer
                dim = x.shape[1]
                eye = torch.eye(dim, device=x.device).unsqueeze(0).repeat(batch_size, 1, 1)

                # Mask identity matrix
                mask_expanded = self.full_mask.unsqueeze(1).float()
                eye = eye * mask_expanded
